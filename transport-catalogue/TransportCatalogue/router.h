...
public:
    explicit Router(const Graph& graph); // конструктор маршрутизатора
    ...
    std::optional<RouteInfo> BuildRoute(VertexId from, VertexId to) const; // построить маршрут
...
private:
    ...
    void InitializeRoutesInternalData(const Graph& graph) { // инициализация данных маршрута ???
