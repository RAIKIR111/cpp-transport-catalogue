...
void TransportCatalogue::AddStop(const string_view name, const geo::Coordinates& coordinates = geo::Coordinates{}) {
    if (stopname_to_stop_.count(name)) {
        ...
        stops_.push_back({string(name), coordinates});
...
void TransportCatalogue::AddBus(const string_view name, const vector<string_view>& entry_route, const bool is_roundtrip, bool is_serialized) {
    vector<Stop*> dst_route;
    if (is_roundtrip == false && is_serialized == false) {
    ...
    buses_.push_back(Bus{string(name), dst_route});
...
const std::unordered_map<std::string_view, domain::Stop*, domain::StringViewHasher>& TransportCatalogue::GetStopnameToStop() const {
    return stopname_to_stop_;
}

const std::unordered_map<std::string_view, domain::Bus*, domain::StringViewHasher>& TransportCatalogue::GetBusnameToBus() const {
    return busname_to_bus_;
}

const std::unordered_map<std::pair<domain::Stop*, domain::Stop*>, int, domain::PairStopsHasher>& TransportCatalogue::GetStopsToDistance() const {
