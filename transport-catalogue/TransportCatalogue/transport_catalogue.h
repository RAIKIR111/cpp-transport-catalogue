...
public:    
    ...
    void AddBus(const std::string_view name, const std::vector<std::string_view>& entry_route, const bool is_roundtrip, bool is_serialized = false);
    ...
    const std::unordered_map<std::string_view, domain::Stop*, domain::StringViewHasher>& GetStopnameToStop() const;

    const std::unordered_map<std::string_view, domain::Bus*, domain::StringViewHasher>& GetBusnameToBus() const;

    const std::unordered_map<std::pair<domain::Stop*, domain::Stop*>, int, domain::PairStopsHasher>& GetStopsToDistance() const;
